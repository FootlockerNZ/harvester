...
def launch(domain: Union[str, List[str]], server_address: Tuple[str, int], browser: Union[BrowserEnum, str] = BrowserEnum.CHROME,
           ...
           extensions: str = None, verbose: bool = False, user_data_directory = None):
    ...
    if user_data_directory:
        user_data_dir = f'--user-data-dir={user_data_directory}'
    else:
    ...
    thread = Thread(target=subprocess.run, args=(
        browser_command,), kwargs={'shell':True}, daemon=True)
    thread.start()
    return thread
