...
def ProxyHTTPRequestHandlerWrapper(delay, domain_cache: Dict[str, MITMRecord] = {}, do_not_track=False):
...
class Harvester(object):
    def __init__(self, host='127.0.0.1', port=5000, delay=2000, do_not_track=False):
        self.domain_cache: Dict[str, MITMRecord] = {}
        self.httpd = ThreadingHTTPServer(
            (host, port), ProxyHTTPRequestHandlerWrapper(delay, self.domain_cache, do_not_track))
    ...
    def launch_browser(self, browser: Union[browserModule.BrowserEnum, str] = browserModule.BrowserEnum.CHROME,
                       ...
                       extensions: str = None, verbose: bool = False, user_data_directory=False):
        return browserModule.launch(list(self.domain_cache.keys()),
                                    self.httpd.server_address, browser, restart, width, height, args, extensions, verbose, user_data_directory=user_data_directory)
